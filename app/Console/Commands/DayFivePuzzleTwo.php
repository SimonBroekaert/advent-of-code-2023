...
class DayFivePuzzleTwo extends Command
{
    use SolvesPuzzles;

    protected $signature = '5.2';

    protected $description = 'The lowest location number that corresponds to any of the initial seed ranges is';

    protected $puzzleInputSource = 'day-5_almanac_list.txt';

    public function handle()
    {
        return $this->error('Memory limit exceeded. :\'(');
        ...
        $seeds = $seeds->chunk(2)->map(function ($chunk) {
            return range($chunk->first(), $chunk->first() + $chunk->last());
        })->flatten();
